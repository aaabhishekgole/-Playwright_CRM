...
export function CashlessApprovalPage() {
  const { requests, approveEstimate, transitionStatus, loading, error } = useRequests({
    statuses: ['ESTIMATE_PREPARED', 'CASHLESS_PENDING_APPROVAL', 'CASHLESS_REVISION_REQUIRED', 'CASHLESS_APPROVED'],
  });
  ...
  const queueContent = cashlessQueue.length > 0
    ? cashlessQueue.map((request) => {
  ...
  return (
    <section className="workspace-page">
      ...
      {error ? <div className="workspace-empty"><strong>Unable to load cashless cases</strong><p>{error}</p></div> : null}
      {loading && cashlessQueue.length === 0 ? <div className="workspace-empty"><strong>Loading cashless queue</strong><p>Please wait while cashless cases are fetched.</p></div> : null}

      <div className="stack-grid">{queueContent}</div>
    </section>
  );
}
