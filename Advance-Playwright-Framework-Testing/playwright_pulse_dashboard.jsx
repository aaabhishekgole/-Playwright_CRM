...
const themeStyles = {
  ...
  dark: {
    page: "bg-[#0a0f1d] text-slate-100",
    panel: "bg-[#111827] border-slate-800",
    softPanel: "bg-[#0f172a] border-slate-800",
...
function StatCard({ title, value, subtext, icon: Icon, valueClassName = "", panelClass = "", mutedClass = "" }) {
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
      <Card className={`rounded-2xl border shadow-sm ${panelClass}`}>
        <CardContent className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className={`text-sm ${mutedClass}`}>{title}</p>
    ...
    <div className={`min-h-screen p-4 md:p-8 transition-colors ${styles.page}`}>
      <div className="mx-auto max-w-7xl">
            ...
            <CardContent className="p-6">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Button className="rounded-xl bg-indigo-600 text-white hover:bg-indigo-600/90" onClick={exportSummary}>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
          <StatCard title="Total Tests" value={selectedRun.totalTests} subtext="Executed in current run" icon={BarChart3} panelClass={styles.panel} mutedClass={styles.textMuted} />
          <StatCard title="Passed" value={selectedRun.passed} subtext={`${passRate}% success rate`} icon={CheckCircle2} valueClassName="text-green-500" panelClass={styles.panel} mutedClass={styles.textMuted} />
          <StatCard title="Failed" value={selectedRun.failed} subtext={`${failRate}% failed`} icon={ShieldAlert} valueClassName="text-red-500" panelClass={styles.panel} mutedClass={styles.textMuted} />
          <StatCard title="Skipped" value={selectedRun.skipped} subtext={`${skipRate}% skipped`} icon={SkipForward} valueClassName="text-amber-500" panelClass={styles.panel} mutedClass={styles.textMuted} />
          <StatCard title="Avg. Test Time" value={formatSeconds(selectedRun.avgTestTimeSeconds)} subtext="Average execution speed" icon={Clock3} panelClass={styles.panel} mutedClass={styles.textMuted} />
          <StatCard title="Run Duration" value={formatSeconds(selectedRun.totalDurationSeconds)} subtext="End-to-end suite time" icon={Timer} panelClass={styles.panel} mutedClass={styles.textMuted} />
        </div>
                ...
                <CardContent>
                  ...
                  <div className="-mt-44 flex justify-center">
                    <div className="text-center">
                      <div className="text-4xl font-bold text-indigo-500">{passRate}%</div>
                      <div className={styles.textMuted}>Passed</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
